...
function PelagensPageInner() {
  ...
  const abrirEdicao = (p: Pelagem) => { setEditando(p); setNome(p.nome); setMostrarForm(true) }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setErro('')
    const result = editando
      ? await updatePelagem({ id: editando.id, input: { nome: nome.trim() } })
      : await createPelagem({ input: { nome: nome.trim() } })
  ...
  return (
    <>
      ...
      <DataTable
        columns={columns}
        data={(data?.pelagens as Pelagem[] | undefined) ?? []}
        rowKey={(p) => p.id}
        loading={fetching}
        error={error?.message}
